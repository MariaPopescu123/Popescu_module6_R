...
**Answer Q.1**
“Forecast uncertainty is the range of possible alternate future conditions
predicted by a model. We generate multiple different predictions of the future
because the future is inherently unknown. Uncertainty comes from natural
variability in the environment, imperfect representation of an ecological system in a
model, and error when measuring the system. When generating a forecast,
uncertainty can come from the structure of the model used, the initial conditions of
the model, the parameters of the model, and the data used to drive the model,
among other sources.”
...
**Answer Q.2**
If there is low uncertainty in a toxic algal bloom forecast, they may prepare
for the possibility of a smaller bloom rather than loading up on supplies to address a
more certain large toxic algal bloom forecast. They may also inform the public more
aggressively if there is a higher certainty of a toxic algal bloom forecast.
...
**Answer Q.4**
There are important effects on water quality variables such as dissolved oxygen,
nutrient and heavy metal concentrations, and algae concentrations.
...
**Answer Q.5**
Yes, as air temperature increases, water temperature increases,
with air temperature being a couple degrees colder than water temperature.
...
**Answer Q.6**
Each grey line is an ensemble member. All the grey lines together are an ensemble. They are predictions of (forecasted) air temperatures from NOAA. 
...
**Answer Q.7**
The multiple linear regression model uses yesterday's water temperature and today's air temperature to predict today's water temperature. Essentially an equation is being solved to determine today's water temperature and the model is finding the y intercept and determining the coefficients for yesterday's water temp and today's air temp that can be applied to to determine today's water temp. 
...
**Answer Q.8**
The fitted model parameters (wtemp_yday and airt) have unequal coefficients (0.81 and 0.18, respectively) indicating that wtemp_yday drives current water temperature more than airt (which makes sense, because yesterday's water temp has a stronger relationship to today's water temperature in comparison to today's air temp). The parameter SE reveal the we have more confidence in wtemp_yday (0.020) and air temp (0.019) than we do in the intercept (0.446). 
...
**Answer Q.9** 
r2 = 0.9, err = 1.342986e-15, rmse = 0.61 
The R2 value indicates that 90% of variance in water temperature can be explained by the model. 
Err is very low, indicating the bias isn't very large.
RMSE is reporting that the average prediction difference from the model daily is 0.61 degrees celcius. For water temperature it is not atrociously bad, but it still shows we are not capturing what is happening perfectly. This will feed into process uncertainty later. 
...
**Answer Q.11** 
It does predict an increase followed by a decrease, but the rate at which it increases is much faster for water temp than for the NOAA forecasts. Probably because as the model is currently, the previous day's water temperature influences the forecast more than the NOAA forecast (the coefficient is much higher 0.81 vs 0.18. 

### What is wrong with deterministic forecasts?

Using a **deterministic** forecast (e.g. a forecast which is one single line, with no uncertainty) is likely to be wrong (even if only slightly) because it ignores the uncertainty that is inherently associated with the future.
...
**Answer Q.13** 
It helps us understand that with time there is a greater range of possible outcomes for that day (and we are less certain about what the value will be).
...
**Answer Q.14**
More observations can help constrain param_se reducing parameter uncertainty. 
...
**Answer Q.15**
sensor precision/accuracy and not capturing full breadth of time in a system can increase initial conditions uncertainty. 
...
**Answer Q.16**
Parameter uncertainty contributes the most to total forecast uncertainty. 
...
*Driver uncertainty* This would likely increase since more drivers means more uncertainty for the those drivers. 

*Parameter uncertainty* This would likely increase since additional variables, would mean additional sources of uncertainty for the parameters of each variable. 

*Initial conditions uncertainty* This wouldn't change, because adding solar radiation and precipitation wouldn't affect water temperature IC uncertainty. 

*Process uncertainty* This would possibly decrease since adding these two variables would help constrain (and explain) the model better (and overall uncertainty would likely decrease as well).
...
```{r}
fit <- lm(model_data$wtemp ~ model_data$wtemp_yday + model_data$airt + model_data$wnd) #added wnd
...
**Answer Q.19**
The multiple linear regression model has one y-intercept and multiple coefficients to predict water temperature. Each coefficient is associated with either wtemp_yday, airt, or wnd. The difference is now wind has been added. 
...
for(d in 2:length(forecasted_dates)) {
  ...
  #pull lagged water temp values
    temp_lag <- forecast_process_unc2 %>%
      filter(forecast_date == forecasted_dates[d-1])

  #run model - REVISE THIS!! done
    temp_pred$value <- coeffs[1] + temp_lag$value * coeffs[2] + temp_driv$air_temperature * coeffs[3] + temp_driv$wind_speed * coeffs[4] + rnorm(n = n_members, mean = 0, sd = sigma)
...
**Answer Q.25**
Parameter uncertainty contributes the most to total forecast uncertainty for my model. 
...
**Answer Q.26** 
Compare the amount of total forecast uncertainty over time: Total forecast uncertainty increased for my model vs the multiple regression model fitted in module6.

Compare the contribution of driver uncertainty: Driver uncertainty looks about the same for both models. But the scale is different. It has probably increased just a little bit since there is an additional driver.         

Compare the contribution of parameter uncertainty: It also increased because I added an additional driver, so that means additional parameter uncertainty associated with that driver.      

Compare the contribution of process uncertainty: Process uncertainty decreased, probably because the additional driver helped constrain the model more.     

Compare the contribution of initial conditions uncertainty: Initial condition uncertainty remained small, but we didn't alter anything in the model to change initial condition uncertainty!     
...
**Answer Q.27**
For this model, I would try to reduce parameter uncertainty since it is the greatest contributor of uncertainty. I can do that with more data points and observations, because my standard errors would decrease as my sample size increases. Once I am able to better constrain my parameter uncertainty I would add additional drivers like precipitation, inflow or outflow to reduce process uncertainty. 
